...
void State::RemoveOccupation(const ShuntingUnit *su)
{
	...
	auto it = find_if(occ.begin(), occ.end(), [su](const ShuntingUnit *s) -> bool
					  { return *su == *s; });
	if (it != occ.end())
	{
		occ.erase(it);
...
{
	auto it = find_if(incomingTrains.begin(), incomingTrains.end(), [incoming](const Incoming *inc) -> bool
					  { return *inc == *incoming; });
...
{
	auto it = find_if(outgoingTrains.begin(), outgoingTrains.end(), [outgoing](const Outgoing *out) -> bool
					  { return *out == *outgoing; });
...
void State::RemoveShuntingUnit(const ShuntingUnit *su)
{
	...
	auto it = find_if(shuntingUnits.begin(), shuntingUnits.end(), [su](const ShuntingUnit *s) -> bool
					  { return *su == *s; });
...
const ShuntingUnit *State::GetMatchingShuntingUnit(const ShuntingUnit *su) const
{
	...
	if (it != shuntingUnitStates.end())
		return it->first;
	auto it2 = find_if(shuntingUnits.begin(), shuntingUnits.end(), [su](const ShuntingUnit *s) -> bool
					   { return su->MatchesShuntingUnit(s); });
	if (it2 != shuntingUnits.end())
		return *it2;
...
bool State::IsActive() const
{
	...
	{
		if (HasActiveAction(su) || IsWaiting(su))
			return true;
	}
	return false;
}
...
void State::RemoveActiveAction(const ShuntingUnit *su, const Action *action)
{
	ce(
		...
		auto it = find_if(lst.begin(), lst.end(), [action](const Action *a) -> bool
						  { return *a == *action; });
		if (it != lst.end()) {
			lst.erase(it);
		})
...
const vector<const Track *> State::GetReservedTracks() const
{
	...
	{
		if (trackState.reserved)
			reserved.emplace_back(track);
	}
	return reserved;
}
...
int State::GetPositionOnTrack(const ShuntingUnit *su) const
{
	...
	auto it = find_if(sus.begin(), sus.end(), [su](const ShuntingUnit *s) -> bool
					  { return *su == *s; });
...
void State::PrintStateInfo() const
{
	...
	for (auto &[su, suState] : GetShuntingUnitStates())
	{
		...
		for (auto &train : su->GetTrains())
		{
			...
			if (tasks.size() + activeTasks.size() > 0)
				...
				cout << ": no tasks.";
			if (tasks.size() > 0)
				cout << "Tasks: " << Join(tasks, ", ");
			if (tasks.size() > 0 && activeTasks.size() > 0)
				cout << " / ";
			if (activeTasks.size() > 0)
				cout << "Active: " << Join(activeTasks, ", ") << endl;
			cout << endl;
		}
	}
	if (shuntingUnits.size() > 0)
	...
	for (auto &[track, trackState] : trackStates)
	{
		if (trackState.reserved || trackState.occupations.size() > 0)
		{
			...
			// 		cout << "Front train " << frontTrain << endl;
			// 	}
			// 	cout << "-------------------------------------------" << endl;
			...
			// 				break;
			// 			}
			// 				// cout << " Combination Action found !";

			// 		}

			// 		if (direction && trains.size()>1)
			// 		{
			// 			auto _state = GetShuntingUnitState(su);
			// 			if(_state.size())
			// 		}
			...
			// 		}
			// 		cout << " ) -";
			// 		cout << " " << (direction ? "Aside " : "Bside ") << endl;
			// 	}
			// 	cout << "-> B";
